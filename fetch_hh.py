...
def process_vacancy_statistics_hh(programm_languages):
    ...
    for programm_language_vacancies, all_vacancies_program_language in hh_vacancies.items():
        vacancies_found = len(all_vacancies_program_language)
        vacancies_processed, average_salary = calculate_average_salary_hh(all_vacancies_program_language)
...
def main():
    parser = argparse.ArgumentParser(
        description='Программа собирает статистику по вакансиям с headhunter.ru'
    )
    parser.add_argument(
        '-pl',
        '--program_language',
    ...
    programming_language_statistics = process_vacancy_statistics_hh(programming_languages)
